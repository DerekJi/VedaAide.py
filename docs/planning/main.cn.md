...
**核心价值**：
- **实际应用**：作为面试准备的探索工具，学习 RAG、Agent 工作流和 LLM 工程化的完整实现
...
- 准备可公开使用的示例数据集（模拟简历、招聘广告、英语题目等）
- 使用 LlamaIndex 实现个人简历、项目文档的结构化清洗与入库（支持用户自有数据或公开数据集）
...
**Kubernetes & Skaffold 设置**
- 配置 Dockerfile 多阶段构建（开发环境、生产环境）
- 编写 Kubernetes 部署清单（Deployment、Service、ConfigMap、Secret）
- 设置 Skaffold 自动构建、推送、部署流程
- 配置本地 Kind 集群用于开发和测试
- 设置容器镜像仓库（GitHub Container Registry 或 Azure Container Registry）

**CI/CD 流水线**
- GitHub Actions 工作流：代码推送 → 单元测试 → 集成测试 → 镜像构建推送 → 自动部署
- 环境隔离：开发环境（Kind）→ 测试环境（云端 K8s）→ 生产环境
- 配置秘钥管理：Azure KeyVault 集成，安全的环境变量注入
- 部署前验证：健康检查、烟雾测试、性能基线检查

**可观测性与成本监控**
- 配置 Prometheus + Grafana 监控（可选）
- 设置 Azure 成本告警
- 部署日志收集（容器日志流向 LangFuse 或 ELK）
- 配置自动扩展（HPA）和资源限制
