...
Usage:
    python translate_with_ollama.py <directory> [--model MODEL] [--lang LANG] [--output-dir OUTPUT_DIR]

Example:
    python translate_with_ollama.py .github/prompts --model deepseek-r1:32b --lang en
    python translate_with_ollama.py .github/prompts --output-dir .temp  # Output to .temp instead of overwriting
"""
...
def translate_text(text: str, model: str, target_lang: str = "en", chunk_size: int = 2000, verbose: bool = False) -> str:
    """
...
def translate_file(file_path: Path, model: str, target_lang: str = "en", dry_run: bool = False, output_dir: Optional[Path] = None, verbose: bool = False) -> bool:
    """
    ...
    try:
        ...
        if translated_content == content:
            print(f"  ⚠️  Warning: No changes detected (translation may have failed)")
...
Examples:
  ...
  python translate_with_ollama.py .github/prompts --verbose
    ...
    parser.add_argument(
        "--ext",
        default=".md",
        help="File extension to translate (default: .md)"
    )
    parser.add_argument(
        "--output-dir",
        default=None,
    ...
    for i, file_path in enumerate(files_to_translate, 1):
        ...
        if translate_file(file_path, model, args.lang, args.dry_run, Path(args.output_dir) if args.output_dir else None, args.verbose):
            success_count += 1
