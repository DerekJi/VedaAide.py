...
def translate_text(text: str, target_lang: str = "en", verbose: bool = False) -> str:
    ...
    try:
        ...
        if verbose:
            print(f"    DEBUG: Original length: {len(text)}, Translated length: {len(translated)}", file=sys.stderr)
...
def translate_file(file_path: Path, target_lang: str = "en", dry_run: bool = False, output_dir: Optional[Path] = None, verbose: bool = False) -> bool:
    """
...
Examples:
  ...
  python translate.py .github/prompts --verbose
    ...
    for i, file_path in enumerate(files_to_translate, 1):
        ...
        if translate_file(file_path, args.lang, args.dry_run, Path(args.output_dir) if args.output_dir else None, args.verbose):
            success_count += 1
