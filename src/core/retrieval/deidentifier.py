...
This module provides unified deidentification functionality for PII (Personally Identifiable Information)
including SSN, phone numbers, email addresses, and addresses.
...
class Deidentifier:
    ...
    PATTERNS = {
        SensitiveInfoType.SSN: re.compile(
    ...
    ADDRESS_KEYWORDS = {
        ...
        'drive', 'dr', 'circle', 'lane', 'ln', 'court', 'ct', 'suite', 'ste',
        'building', 'bldg', 'floor', 'apt', 'apartment', 'zip', 'postal',
        'city', 'state', 'county', 'province', 'district', 'town'
    }
    ...
    def __init__(self, 
        ...
        self._detection_stats = {
            info_type: 0 for info_type in SensitiveInfoType
        }
    ...
    def _detect_addresses(self, text: str) -> List[SensitiveMatch]:
        ...
        street_pattern = re.compile(
            r'\d+\s+[A-Z][a-z]+(?:\s+(?:' + '|'.join(self.ADDRESS_KEYWORDS) + r'))',
            re.IGNORECASE
        )
        ...
        for match in street_pattern.finditer(text):
            ...
            if any(keyword in context for keyword in ['address', 'located', 'lives', 'resident', 'at']):
                sensitive_match = SensitiveMatch(
    ...
    def verify_deidentification(self, original: str, deidentified: str) -> bool:
        ...
        for match in matches:
            ...
            if match.type in [SensitiveInfoType.SSN, SensitiveInfoType.EMAIL, 
                            SensitiveInfoType.PHONE, SensitiveInfoType.CREDIT_CARD]:
                sensitive_values.append(match.text)
        
        # Check if any sensitive value appears in deidentified text
        for value in sensitive_values:
            if value in deidentified:
                logger.warning(f"Verification failed: sensitive value '{value}' found in deidentified text")
                return False
